...
void HttpResponse::sendFile(QString filename, QString mimeType, QString charset, int len, int compressionLevel,
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        if (config->verbosity >= HttpServerConfig::Verbose::Info)
            qInfo().noquote()
                    << QString("Unable to open file to be sent (%1): %2").arg(filename).arg(file.errorString());
...
void HttpResponse::setHeader(const QString& name, const QString& value, bool encode) {
    headers[name] = encode ? QUrl::toPercentEncoding(value) : value;
}

void HttpResponse::setHeader(const QString& name, const QDateTime& value) {
