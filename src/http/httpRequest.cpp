...
HttpRequest::HttpRequest(HttpServerConfig *config) : config(config), buffer(), requestBytesSize(0),
                                                     state_(State::ReadRequestLine), method_(), uri_(), version_(),
                                                     expectedBodySize(0), body_(), mimeType_(),
...
bool HttpRequest::parseRequestLine(QTcpSocket *socket, HttpResponse *response) {
    ...
    if (parts.length() != 3 || !parts[2].startsWith("HTTP")) {
        if (config->verbosity >= HttpServerConfig::Verbose::Info) {
            qInfo().noquote()
                    << QString("Invalid HTTP request line received from %1: %2").arg(socket->peerAddress().toString())
    ...
    if (!uri_.isValid()) {
        if (config->verbosity >= HttpServerConfig::Verbose::Info)
            qInfo().noquote()
                    << QString("Invalid URI received from %1: %2").arg(address_.toString()).arg(QString(parts[1]));
    ...
    if (field.compare("Cookie", Qt::CaseInsensitive) == 0) {
        ...
        for (QString part: parts) {
            ...
            if (kvPart.length() != 2) {
                if (config->verbosity >= HttpServerConfig::Verbose::Info)
                    qInfo().noquote()
                            << QString("Invalid cookie header for %1: %2").arg(address_.toString()).arg(value);
    ...
    if (charset_.compare("US-ASCII", Qt::CaseInsensitive) == 0 ||
        ...
        if (!codec) {
            ...
            if (config->verbosity >= HttpServerConfig::Verbose::Warning)
                qWarning().noquote()
                        << QString("Unknown charset when parsing body: %1. Falling back to UTF-8").arg(charset_);
